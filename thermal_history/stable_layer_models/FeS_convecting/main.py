...
#*# evolve method to account for FeS layer that is convecting
def evolve(model):
...
def conducting_FeS(model):
    ...
    if sl.layer_thickness > prm.FeS_size:
        ...
        dT_dt_fes = Qs_fes / (-4*np.pi*trapezoid(r_prof_fes, prm.FeS_density * prm.FeS_cp * (T_prof_fes/T_prof_fes[0]) * r_prof_fes**2)[-1])

    else:

        dT_dt_fes = core.dT_dt*(T_prof_fes[0]/core.Tcen)
    ...
    while time_gone < model.dt:
        ...
        #*# New boolean, hold layer thickness constant
        if sl.hold and sl.layer_thickness == prm.FeS_size:
            lb, ub = 0,0
            time_gone = model.dt
            r = np.array([r_fes, r_fes])
            T = prof.adiabat(r, model.core.Tcen + dTa_dt*model.dt, prm.core_adiabat_params)

        #!# Only solve for bulk if ADR<1
        elif sl.ADR < 1:
            ...
            #Initialise layer with minimum tolerance thickness
            ...
            #Regrid core radial profiles onto finer stable layer grid. #*# Just interpolate onto bulk profiles to avoid FeS values.
            ...
            #*# Hold FeS layer temperature by stopping it from cooling just on the iteration a layer tries to grow.
            if sl.layer_thickness == prm.FeS_size:
                dT_dt_fes = 0


            #*# Upper bc based on FeS cooling rate
            ub_type, ub = 0, T_prof_fes[0] + dT_dt_fes*(time_gone+dt_small)

            #Calculate diffusion solution

            #*# Just the bulk. diffusion_uneven() gives same results as diffusion() from leeds_thermal if
    ...
    #*#
    sl._next_profiles['r'] = np.append(r_prof_bulk, r_prof_fes)
    sl._next_profiles['T'] = np.append(T_prof_bulk, sl.profiles['fes']['T'])
